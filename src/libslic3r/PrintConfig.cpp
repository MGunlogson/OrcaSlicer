...
void PrintConfigDef::init_fff_params()
{
    ...
    */
    ...
    def->mode = comAdvanced;
    def->set_default_value(new ConfigOptionEnum<MagmaTubeSolverMode>(MagmaTubeSolverMode::Refined));

    def = this->add("magma_solver_timeout", coFloat);
    def->label = L("Solver timeout");
    def->category = L("Strength");
    def->tooltip = L("Maximum time in seconds the optimizer spends on each block of cells. "
                     "Higher values may improve tube coverage on complex models at the cost "
                     "of longer slicing time. Only used in Refined solver mode.");
    def->sidetext = L("s");
    def->min = 1;
    def->max = 120;
    def->mode = comAdvanced;
