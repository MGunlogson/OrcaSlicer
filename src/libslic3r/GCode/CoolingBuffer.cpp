...
struct CoolingLine
{
    enum Type {
        ...
        // Magma injection fan speed control
        TYPE_MAGMA_INJECTION_FAN_START = 1 << 21,
        TYPE_MAGMA_INJECTION_FAN_END   = 1 << 22,
    };
...
std::vector<PerExtruderAdjustments> CoolingBuffer::parse_layer_gcode(const std::string &gcode, std::vector<float> &current_pos) const
{
    ...
    for (; *line_start != 0; line_start = line_end)
    {
        ...
        } else if (boost::starts_with(sline, ";_MAGMA_INJECTION_FAN_START")) {
            line.type = CoolingLine::TYPE_MAGMA_INJECTION_FAN_START;
        } else if (boost::starts_with(sline, ";_MAGMA_INJECTION_FAN_END")) {
            line.type = CoolingLine::TYPE_MAGMA_INJECTION_FAN_END;
    ...
    std::vector<PerExtruderAdjustments>    &per_extruder_adjustments)
{
    ...
    bool magma_injection_fan_control = false;
    int  magma_injection_fan_speed   = 0;
    auto change_extruder_set_fan = [ this, layer_id, layer_time, &new_gcode,
        ...
        &magma_injection_fan_control, &magma_injection_fan_speed
...
#undef EXTRUDER_CONFIG
            ...
            magma_injection_fan_control = false;
            magma_injection_fan_speed = 0;
        }
    ...
    std::unordered_map<int, bool> fan_speed_change_requests = {{CoolingLine::TYPE_OVERHANG_FAN_START, false},
                                                               ...
                                                               {CoolingLine::TYPE_MAGMA_INJECTION_FAN_START, false},
    ...
    for (const CoolingLine *line : lines) {
        ...
        if (line->type & CoolingLine::TYPE_SET_TOOL) {
            ...
            need_set_fan = true;
        } else if (line->type & CoolingLine::TYPE_MAGMA_INJECTION_FAN_START) {
            if (magma_injection_fan_control && !fan_speed_change_requests[CoolingLine::TYPE_MAGMA_INJECTION_FAN_START]) {
                fan_speed_change_requests[CoolingLine::TYPE_MAGMA_INJECTION_FAN_START] = true;
                need_set_fan = true;
            }
        } else if (line->type & CoolingLine::TYPE_MAGMA_INJECTION_FAN_END && fan_speed_change_requests[CoolingLine::TYPE_MAGMA_INJECTION_FAN_START]) {
            if (magma_injection_fan_control) {
                fan_speed_change_requests[CoolingLine::TYPE_MAGMA_INJECTION_FAN_START] = false;
            }
            need_set_fan = true;
        ...
        if (need_set_fan) {
            ...
            else if (fan_speed_change_requests[CoolingLine::TYPE_MAGMA_INJECTION_FAN_START]){
                new_gcode += GCodeWriter::set_fan(m_config.gcode_flavor, magma_injection_fan_speed);
                m_current_fan_speed = magma_injection_fan_speed;
            }
