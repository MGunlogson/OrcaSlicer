	...
	for (size_t region_id = 0; region_id < layer.regions().size(); ++ region_id) {
	    ...
	    for (const Surface &surface : layerm.fill_surfaces.surfaces)
	        if (surface.surface_type == stInternalVoid)
		        ...
		        // stZoneOuter passes is_solid() (injection fills tubes before the next layer),
		        // but for FlowRole/extruder selection it should use frInfill — the outer zone
		        // has its own filament (dual_infill_outer_filament) routed via has_zone_fill()
		        // in ToolOrdering, not through the solid infill extruder.
		        FlowRole extrusion_role = surface.is_top() ? frTopSolidInfill
		            : (surface.is_zone_outer() ? frInfill
                ...
                if (surface.is_zone_outer()) {
                    ...
                    // Keep sparse density - injection fills tubes per-layer during printing
