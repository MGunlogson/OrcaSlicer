...
// Uses min_layer_height for conservative mm→layers conversion (more layers = wider window).
...
struct WindowSpec {
    ...
    // min_layer_height: smallest layer height in the object, for conservative mm→layers conversions
    static WindowSpec from_config(
        ...
        float layer_height,                 // nominal layer height for conversions
        int config_stagger_levels,          // number of stagger groups
        float min_layer_height              // for conservative mm→layers conversions
    );
