...
int calculate_auto_window_height(double interior_width, double line_width, double layer_height)
{
    ...
    int layers = static_cast<int>(std::ceil(window_height_mm / layer_height)) + 1;
    return std::max(2, layers);  // Minimum 2 layers
}

// Calculate effective tube height from both config values (layers and mm)
// Whichever requires more layers wins
static int calculate_tube_height_layers(int config_layers, float config_mm, float layer_height)
{
    int from_layers = config_layers;
    int from_mm = (config_mm > 0) ? static_cast<int>(std::ceil(config_mm / layer_height)) : 0;
    ...
    int config_stagger_levels)
{
    ...
    if (config_window_height <= 0) {
        spec.window_height_layers = calculate_auto_window_height(interior_width, line_width, layer_height);
    } else {
    ...
    spec.tube_height_layers = calculate_tube_height_layers(config_tube_height_layers, config_tube_height_mm, layer_height);
