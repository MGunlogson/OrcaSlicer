...
private:
    ...
    // Per-layer data for adaptive layer height support.
    // Indexed by layer_id. Built in build() from the layers vector.
    std::vector<double> m_print_z;        // print_z (cumulative) per layer_id
    std::vector<double> m_layer_heights;  // individual height per layer_id
    ...
    int    m_min_tube_height_layers;  // approximate, for logging
    int    m_max_tube_height_layers;  // approximate, for logging
    int    m_num_layers;
