...
void LayerRegion::process_external_surfaces(const Layer *lower_layer, const Polygons *lower_layer_covered)
{
    ...
    // Zone outer is expendable like sparse (bridges/top/bottom can consume it).
    enum : size_t { zShells = 0, zSparse = 1, zZoneOuter = 2 };
    std::vector<ExpansionZone> expansion_zones{
    ...
    // Now: zShells, zSparse, zZoneOuter
    ...
    if (!this->layer()->object()->print()->config().spiral_mode && this->region().config().sparse_infill_density.value > 0) {
        ...
        move_small_to_solid(expansion_zones[zZoneOuter].expolygons, small_to_solid);
        ...
        if (this->region().config().filter_narrow_sparse_infill.value) {
            ...
            if (thin_threshold > 0) {
                ...
                for (size_t zone_idx : {zSparse, zZoneOuter}) {
    ...
    }
    {
        ...
        this->fill_surfaces.append(std::move(expansion_zones[zZoneOuter].expolygons), zone_outer_templ);
    }
