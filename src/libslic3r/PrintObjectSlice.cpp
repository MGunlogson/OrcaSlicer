...
void PrintObject::compute_zone_boundary()
{
    ...
    if (min_inner_width > 0) {
        ...
        zone_boundary::regenerate_mesh_from_grid(*m_zone_interior);
        m_zone_stages.filtered = sla::get_mesh(*m_zone_interior);
    } else {
        // No filtering, filtered stage same as initial
        m_zone_stages.filtered = m_zone_stages.initial;
    }
