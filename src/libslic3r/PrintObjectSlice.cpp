...
#include "Print.hpp"
#include "Magma/MagmaInterior.hpp"
//BBS
...
// Magma: Compute 3D shell using SLA hollowing infrastructure and slice it
void PrintObject::compute_magma_shell()
{
    // Clear old stage meshes before computing new ones
    // This prevents stale data from persisting when re-slicing
    m_magma_stages.initial.clear();
    m_magma_stages.filtered.clear();
    m_magma_stages.smoothed.clear();

    // Check if Magma is enabled for any region of this object
    bool magma_enabled = false;
    for (const auto &region_ref : this->all_regions()) {
        if (region_ref.get().config().magma_inner_shell_enabled) {
            magma_enabled = true;
            break;
        }
    }
    if (!magma_enabled || m_layers.empty())
        return;

    // Get Magma config from first enabled region
    double outer_width = 0;
    double min_yolk_width = 0;
    for (const auto &region_ref : this->all_regions()) {
        if (region_ref.get().config().magma_inner_shell_enabled) {
            outer_width = region_ref.get().config().magma_outer_infill_width.value;
            min_yolk_width = region_ref.get().config().magma_min_yolk_width.value;
            break;
        }
    }
    if (outer_width <= 0)
    ...
    // Report progress: Magma hollowing started
    m_print->set_status(6, L("Magma: Computing 3D shell"));
    m_print->throw_if_canceled();
    ...
    m_magma_interior = sla::generate_interior(mesh, cfg);
    m_print->throw_if_canceled();

    if (!m_magma_interior || sla::get_mesh(*m_magma_interior).empty())
        return;

    // Magma: Capture initial stage mesh for debug visualization
    m_magma_stages.initial = sla::get_mesh(*m_magma_interior);
    ...
    if (min_yolk_width > 0) {
        m_print->set_status(7, L("Magma: Filtering thin sections"));
        m_print->throw_if_canceled();
        magma::filter_thin_interior(*m_magma_interior, min_yolk_width);
        // Regenerate mesh from filtered grid to capture filtered stage
        magma::regenerate_mesh_from_grid(*m_magma_interior);
        m_magma_stages.filtered = sla::get_mesh(*m_magma_interior);
    } else {
        // No filtering, filtered stage same as initial
        m_magma_stages.filtered = m_magma_stages.initial;
    }

    // Report progress: smoothing
    m_print->set_status(8, L("Magma: Smoothing shell"));
    m_print->throw_if_canceled();
    ...
    magma::smooth_interior(*m_magma_interior, mesh, smoothing_iterations);
    m_print->throw_if_canceled();

    if (sla::get_mesh(*m_magma_interior).empty())
        return;

    // Magma: Capture smoothed stage mesh for debug visualization
    m_magma_stages.smoothed = sla::get_mesh(*m_magma_interior);

    // Report progress: slicing shell
    m_print->set_status(10, L("Magma: Slicing shell"));
    m_print->throw_if_canceled();

    // Get the interior mesh and slice it at same layer heights
    indexed_triangle_set interior_its = sla::get_mesh(*m_magma_interior);
    ...
    for (size_t i = 0; i < m_layers.size(); ++i)
        m_layers[i]->magma_zone_boundary = std::move(interior_slices[i]);
}
