...
void ConfigManipulation::update_print_fff_config(DynamicPrintConfig* config, const bool is_global_config, const bool is_plate_config)
{
    ...
    if (auto* z_slam_opt = config->option<ConfigOptionFloat>("magma_injection_z_slam");
        z_slam_opt && z_slam_opt->value > 2.0 + EPSILON)
    {
        const wxString msg_text = _(L("Z-slam depth beyond 2mm is very aggressive.\n\n"
...
//    toggle_line("support_interface_speed", have_support_material && have_support_interface);
    ...
    // Refined-only settings: dodge distance and solver timeout
